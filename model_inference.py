...
# This makes paths relative to the script itself, which is robust for deployment.
...
# It should be in the project root, so relative to BASE_DIR
...
try:
    if os.path.exists(MODEL_PATH): # Added check to see if file exists before trying to load
...
try:
    # First, try the explicit path we defined based on BASE_DIR
    FACE_CASCADE = cv2.CascadeClassifier(HAARCASCADE_PATH)
    if FACE_CASCADE.empty():
        # Fallback to OpenCV's default data path, though not guaranteed on all deployments
        print(f"Warning: Haar Cascade not found at {HAARCASCADE_PATH}. Attempting OpenCV default path.")
        FACE_CASCADE = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
        if FACE_CASCADE.empty():
            raise IOError(f"Haar Cascade classifier not loaded from {HAARCASCADE_PATH} or OpenCV default path. Please ensure 'haarcascade_frontalface_default.xml' is in your project root.")
...
def detect_and_predict_faces(image_path):
    ...
    for (x1, y1, x2, y2) in picked_boxes:
        ...
        cropped_face_cv = img_cv[crop_y_min:crop_x_max, crop_y_min:crop_y_max] # Changed y_max, x_max here
        # Corrected order: [rows (height), cols (width)]
        cropped_face_cv = img_cv[crop_y_min:crop_y_max, crop_x_min:crop_x_max] # Reverted for clarity
